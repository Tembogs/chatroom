...
export default function ClientDashboard() {
  ...
  useEffect(() => {
    const sync = async () => {
      try {
        const res = await api.get('https://contact-support.onrender.com/api/request/accepted');
        const savedUser = JSON.parse(localStorage.getItem('user') || '{}');
        const myId = savedUser.id;

        if (myId) {
          const profileRes = await api.get(`https://contact-support.onrender.com/api/profile/user/${myId}`);
          setClient(profileRes.data); 
        }
        if (res.data && res.data.length > 0) {
          const req = res.data[0];
          setActiveRequest(req);
          const clientRes = await api.get(`https://contact-support.onrender.com/api/profile/user/${req.userId}`);
  ...
  const handleCreateRequest = async () => {
    try {
      setIsSearching(true);
      const res = await api.post('https://contact-support.onrender.com/api/request'); 
  ...
  const handleCancelRequest = async () => {
    ...
    try {
      await api.post(`https://contact-support.onrender.com/api/request/${activeRequest.id}/cancel`);
      setActiveRequest(null);
      setMessages([]);
      setIsSearching(false); 
    } catch (err) {
  ...
  try {
    await api.post('https://contact-support.onrender.com/api/review', { 
      rating, 
      comment, 
